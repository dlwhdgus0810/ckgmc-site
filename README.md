...
## 문의 폼 · 세례 신청 폼
정적 사이트에는 서버가 없으므로 메일 발송은 외부 폼 서비스를 씁니다.
[Formspree](https://formspree.io) (무료) 에서 폼을 만들고 받은 주소를 `src/data/site.ts` 의 `formEndpoint` 에 넣으면
연락처 페이지의 문의 폼과 세례 신청 폼이 그 주소로 전송됩니다. 비워두면 이메일 링크로 대체됩니다.
...
* 문의/세례 신청 폼 — Formspree 같은 폼 서비스 연결 필요 (위 참고).
